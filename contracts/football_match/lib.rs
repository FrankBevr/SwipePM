...
mod football_match {
    ...
    impl GameData {
        ...
        #[ink(message)]
        pub fn set_winner(&mut self, number: i32) -> Result<(), ()> {
            self.winner = number;
            ink::env::debug_println!("Now the winner is set. The Winner is {:?}", self.winner);
            Ok(())
        }
        #[ink(message)]
        pub fn set_particpant_chelsea(&mut self) -> Result<(), ()> {
        ...
        #[ink(message)]
        pub fn set_particpant_manchester(&mut self) -> Result<(), ()> {
